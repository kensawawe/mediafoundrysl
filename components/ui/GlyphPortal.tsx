...
export default function GlyphPortal({
  ...
  const characters = Array.from(text).reduce<{ char: string; index: number }[]>((all, char) => {
    const last = all[all.length - 1];
    all.push({ char, index: last ? last.index + last.char.length : 0 });
    return all;
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/purity -- inside an effect; measures real mount time
